...
def recognize_captcha(str_image_path):
        ...
        str_api_key = "AIzaSyArqFbDmaQ3MB1CQE4q3TdHaXuLoGG2Dqs"
        #gitにあげるときは取り除く
        ...
        for j in range(len(i['textAnnotations'])):
            ...
            if len(temp) != 0:
                ...
                if not arr:
                    ...
                    #print(list)
    print(list)
    #取ってきたリストの確認　テスト用print
    #一度リストに落とし込んでからそのリストの中を判定しに行く

    #この下に判定基準の判別を書いていく

    pork_haram_counter =list.count('豚肉')
    pork_haram_counter = pork_haram_counter + list.count('ポーク')
    pork_haram_counter = pork_haram_counter + list.count('豚')
     #ex)豚肉の検出
...
def show_result(counter):
    ...
    print("pork_ele:{},alcohol_ele:{},counter:{}".format(pork_ele,alcohol_ele,counter))
    if counter ==0:
